...
export default function PostExplorer({
  ...
  const defaultCalendarMonth = useMemo(() => {
    ...
    if (events.length > 0) {
      const firstValid = events.find((e) => !Number.isNaN(new Date(e.created_at).getTime()));
      if (firstValid) return new Date(firstValid.created_at);
    }
    return new Date();
  }, [activeDate, events]);
  ...
  const embed = selected ? youtubeEmbed(selected) : '';
  const xDoc = selected ? xEmbedDoc(selected) : '';
  const thumbnail = selected ? thumbnailUrl(selected) : '';
  const avatar = selected ? avatarUrl(selected) : '';
    ...
    <div className="post-explorer">
      <aside className="post-result-rail">
        ...
        <div className="post-filter-toolbar">
          <button
            type="button"
            className={`post-date-btn ${activeDate ? 'active' : ''}`}
            onClick={() => setShowDatePicker((open) => !open)}
            title="Filter observed posts by publication date"
          >
            <CalendarIcon size={13} />
            <span>{activeDate ? activeDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'All Dates'}</span>
          </button>
          {activeDate ? (
            <button
              type="button"
              className="post-date-clear-btn"
              onClick={() => {
            ...
            >
              Clear filter ({filteredEvents.length})
            </button>
          ) : (
            <span className="post-calendar-hint">{events.length} total</span>
          )}
        </div>

        {showDatePicker && (
          <div className="post-calendar-card">
            <div className="post-calendar-card-header">
              <span>FILTER POSTS BY DATE</span>
              <button type="button" className="text-btn" onClick={() => setShowDatePicker(false)}>✕</button>
            </div>
            <Calendar10
              value={activeDate}
              ...
              className="rounded-lg border border-[#1f2e47] bg-[#090f1a] w-full"
            />
            <div className="post-calendar-card-footer">
              {activeDate ? (
                <button
                  type="button"
                  className="text-btn"
                ...
                <span className="post-calendar-hint">Select a day to filter</span>
              )}
              <button
                type="button"
                className="btn btn-secondary btn-sm"
                style={{ marginLeft: 'auto', height: 26, fontSize: 11, padding: '0 8px' }}
        ...
        <div className="post-result-list">
          {filteredEvents.length === 0 ? (
            <div className="post-empty" style={{ minHeight: 180, padding: 16 }}>
              ...
              <p style={{ margin: '6px 0 10px', fontSize: 11 }}>{events.length} observed posts exist on other dates.</p>
              <button
                type="button"
              ...
              >
                Show all dates ({events.length})
              </button>
            </div>
          ) : (
            filteredEvents.slice(0, 250).map((event) => {
              const thumb = thumbnailUrl(event);
              const isActive = selected && event.id === selected.id;
              return (
